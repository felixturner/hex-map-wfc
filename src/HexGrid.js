...
export class HexGrid {
  ...
  hideAllInstances() {
    ...
    if (this.decorations) {
      ...
      for (const building of this.decorations.buildings) {
        this.decorations.staticMesh.setMatrixAt(building.instanceId, dummy.matrix)
      }
      for (const bridge of this.decorations.bridges) {
        this.decorations.staticMesh.setMatrixAt(bridge.instanceId, dummy.matrix)
      }
      for (const lily of this.decorations.waterlilies) {
        this.decorations.staticMesh.setMatrixAt(lily.instanceId, dummy.matrix)
      }
      for (const flower of this.decorations.flowers) {
        this.decorations.treeMesh.setMatrixAt(flower.instanceId, dummy.matrix)
      }
      for (const rock of this.decorations.rocks) {
        this.decorations.staticMesh.setMatrixAt(rock.instanceId, dummy.matrix)
      }
      for (const hill of this.decorations.hills) {
        this.decorations.staticMesh.setMatrixAt(hill.instanceId, dummy.matrix)
      }
      for (const mountain of this.decorations.mountains) {
        this.decorations.staticMesh.setMatrixAt(mountain.instanceId, dummy.matrix)
      }
    }
  ...
  buildDecorationMap() {
    ...
    // Build a set of windmill fan instanceIds for tagging
    const fanInstanceIds = new Set()
    const fanByInstanceId = new Map()
    for (const fan of this.decorations.windmillFans) {
      fanInstanceIds.add(fan.instanceId)
      fanByInstanceId.set(fan.instanceId, fan)
    }
    ...
    for (const building of this.decorations.buildings) {
      ...
      // Tag windmill fan so animateDecoration can resume spin tween
      const fan = fanByInstanceId.get(building.instanceId)
  ...
  animateDecoration(items) {
    ...
    for (const item of list) {
      ...
      gsap.to(anim, {
        ...
        onComplete: () => {
          // Resume windmill fan spin after drop-in finishes
